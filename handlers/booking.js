const { createBooking, getUserBookings, cancelBooking, getBookingsForOwner } = require('../services/booking');
...
const getForOwner = async (req, res) => {
  try {
    const bookings = await getBookingsForOwner(req.userId);
    res.status(200).json({ count: bookings.length, bookings });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
module.exports = { create, getMyBookings, cancel, getForOwner };